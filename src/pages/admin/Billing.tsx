...
export default function Billing() {
  const transactions = [
    ...
    { id: 'TX-9918', user: 'Dewi Lestari', amount: 'Rp 88.500', status: 'completed', date: 'Yesterday' },
  ];
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-headline font-bold">Billing & Revenue</h2>
          <p className="text-sm text-slate-500">Manage financial transactions and invoice cycles.</p>
        </div>
      ...
      <section className="bg-gradient-to-br from-tertiary to-tertiary-container rounded-[2.5rem] p-8 text-white shadow-xl shadow-tertiary/20">
        <div className="flex justify-between items-start">
          <div>
            <p className="text-white/80 text-[10px] font-bold uppercase tracking-widest mb-2">Total Revenue (Monthly)</p>
            <h3 className="text-4xl font-headline font-extrabold">Rp 1.24B</h3>
            <p className="text-sm mt-2 flex items-center gap-1">
              <TrendingUp size={16} />
              <span className="font-bold">+12.4%</span> vs last month
            </p>
          </div>
          ...
          {transactions.map((tx, i) => (
            ...
            >
              ...
              <div className="text-right">
                ...
                <button className="text-[10px] font-bold text-slate-400 hover:text-primary uppercase tracking-widest">Details</button>
              </div>
            </motion.div>
          ))}
        </div>
